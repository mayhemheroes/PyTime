#!/usr/bin/env bash
#
# mayhem/test.sh — RUN PyTime's own test suite (deps already installed by mayhem/build.sh) and
...
# the pytime_run_tests ELF wrapper. We only RUN the suite. The suite is routed through that compiled
...
# Put the in-image site dir (atheris + pytest) and the PyTime source on PYTHONPATH.
